...
```
- [ ] **Step 2: Run it, expect FAIL** — `pytest tests/integration/test_infra_up.py -v` → connection refused.
- [ ] **Step 3: Write `docker-compose.yml`** with zookeeper, kafka, schema-registry (Apicurio), namenode,
...
- [ ] **Step 4: Boot & run** — `docker compose -f infra/docker/docker-compose.yml up -d` then the test → PASS.
- [ ] **Step 5: Commit** — `git commit -m "feat(infra): docker-compose stack with health checks"`
...
- [ ] **Step 1: Failing test** — assert `kafka-topics --list` contains all 4 topics and
  `hdfs dfs -test -d /data/bronze` exits 0.
- [ ] **Step 2: Run, expect FAIL.**
- [ ] **Step 3: Write `bootstrap.sh`** — loop `kafka-topics --create --if-not-exists` reading partition
  counts from `contracts/avro/topics.yml`; `hdfs dfs -mkdir -p` the three zones. Idempotent (re-runnable).
- [ ] **Step 4: Run, expect PASS.**
- [ ] **Step 5: Commit** — `feat(infra): idempotent bootstrap for topics + HDFS dirs`.
...
- [ ] **Step 1: Failing test** — query `http://prometheus:9090/api/v1/targets` and assert every job is `up`.
- [ ] **Step 2: Run, expect FAIL.**
- [ ] **Step 3: Write scrape config** with jobs for kafka-exporter, redis-exporter, spark, and the four
  Python layer apps; add Grafana provisioning + one overview dashboard JSON.
- [ ] **Step 4: Run, expect PASS.**
- [ ] **Step 5: Commit** — `feat(infra): prometheus + grafana provisioning`.
...
- [ ] **Step 1: Failing test** — `helm lint infra/helm/supply-chain` exits 0 (skip if helm absent).
- [ ] **Step 2: Run, expect FAIL.**
- [ ] **Step 3: Write chart** (Deployments/Services for each engine) + Terraform stub with a big
  `# COST WARNING: do not apply against a billed provider` banner and all counts defaulted to free tiers.
- [ ] **Step 4: `helm lint` → PASS.**
- [ ] **Step 5: Commit** — `feat(infra): cloud-ready helm chart + terraform stub (manifests only)`.

### Task 5 (M5): Runbooks

**Files:** `docs/runbooks/{startup-shutdown.md,troubleshooting.md,scaling.md}`
- [ ] Write runbooks: how to `make up/down`, common failures (port clashes, low RAM, HDFS safemode),
